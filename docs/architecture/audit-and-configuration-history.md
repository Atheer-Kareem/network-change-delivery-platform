...
| --- | --- | --- | --- | --- |
| Intent and live deployment request | Git review; one device or fleet change and the explicit live authorization | Git identity; request binds change, plan, assurance, and promotion digests; no credentials | Product repository, durable Git history | Reference repository, commit, PR when available, path, and bound digests |
...
| `AssuranceEvidence` and `PlanAssuranceRecord` | Batfish assurance; behavioral observations bound to plan, policy, and snapshots | The enclosing record is canonical and self-digested; bounded modeled evidence, not credentials | Local mode-0600 JSON and promotion bundle | Reference the record digest and outcome; retain the immutable record as a separate artifact |
| `DeploymentPromotionManifest` and bundle | Promotion step; exact approved artifact inventory | Manifest is canonical and self-digested; each bundled artifact has size and SHA-256 | Buildkite artifact and build metadata; checkout copy is temporary | Reference manifest and bundle digests; Buildkite retention is not durable authority |

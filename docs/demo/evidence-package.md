...
Use the [current schema-v2 acceptance](../acceptance/profiled-main-delivery.md)
as the canonical delivery milestone. Its runtime results are user-supplied;
source/static tests confirm the implemented control chain. This guide does not
require another write or a device reset. Historical evidence below is retained,
not regenerated or presented as the current delivery mechanism.

## Current demonstration — three truthful outcomes
...
| --- | --- | --- | --- | --- | --- | --- |
| Historical pipeline architecture | STATIC HISTORICAL | Buildkite Build #281 | Individually visible validation, `validation-complete`, CML/Batfish fan-out, sanitized annotations, immutable promotion, and the human authorization boundary | A new device write during the demo | Build #281 pipeline timeline and annotations | [Buildkite workflow](../architecture/buildkite-workflow.md) and repository pipeline tests |
| Historical PR staging | STATIC HISTORICAL | Buildkite Build #280 | Disposable create → READ-ONLY validate → destroy and protected delivery ineligible on a PR | Application of the pending live plan or a live write | Build #280 staging job and annotation | [Staging operations](../architecture/buildkite-ephemeral-cml-staging-operations.md) |
...
| Current live topology | DYNAMIC LIVE | CML `NCDP Live` | The declared current proof population is present and booted, if manually verified | Desired-state authority, staging ownership, or proof of a new write | CML topology and node-state UI | MANUAL verification prompted by `ncdp-demo-readiness` |
...
Buildkite retention is not durable evidence authority. Buildkite explains a pipeline
attempt; for the historical integrated path,
AuditStore and ConfigurationObservationStore retain the typed,
...
- `ncdp-demo-readiness` automates local Git, Docker, loopback NetBox/Grafana/
  Prometheus/OpenBao availability, selected historical typed evidence, and an already
  running viewer.

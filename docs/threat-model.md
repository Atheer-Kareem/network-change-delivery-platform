...
7B token deliberately has no secret-read policy. The accepted 7C protected
deployment boundary uses a distinct single-device role, one exact read policy,
one token use, and a commit-changed request bound to the promoted plan. A
compromised eligible deployment job remains a high-impact boundary; short

...
class StagingInventory:
    """Resolve the two logical devices with their admitted staging endpoints."""
    ...
    def __init__(
        ...
        self._devices = {device.name: device for device in devices.values()}

    def resolve(self, target: str, interface: str | None = None) -> InventoryDevice:
        admitted = self._devices.get(target)
        if admitted is None:
            raise StagingError("staging target is not one of the two admitted devices")
        fresh = self._inventory.resolve(target, interface)
        if fresh.inventory_object_id != admitted.inventory_object_id:
            raise StagingError("fresh NetBox identity changed")
        return fresh.model_copy(update={"host": admitted.host})
...
class LocalOperations:
    ...
    def _resolve_authority(self) -> None:
        if self._buildkite_context is not None:
            ...
            token = os.environ.get("NCDP_NETBOX_TOKEN")
            inventory = NetBoxInventoryProvider()
        ...
        netbox_url = os.environ.get("NCDP_NETBOX_URL")
        if not netbox_url or not token:
            raise StagingError("NetBox staging authority is incomplete")
        for role, (
            name,
            live_host,
            staging_cidr,
            ...
            device = inventory.resolve(name, management)
            if (
                device.host != live_host
            ...
            interface_id = device.inventory_interface_object_id
            expected_interface_id = (
                f"netbox:dcim.interface:{1 if role == 'core_02' else 3}"
            )
            if interface_id != expected_interface_id:
                raise StagingError(f"{name} management interface identity mismatch")
            self._verify_secondary_address(
                netbox_url,
                token,
                staging_cidr,
                int(interface_id.rsplit(":", 1)[1]),
            )
            ...
            self._devices[role] = device.model_copy(
                update={"host": staging_cidr.removesuffix("/24")}
            )
        ...
        self._terraform_env.update(
            {
                ...
                "TF_VAR_core_02_bootstrap_management_cidr": "192.168.4.30/24",
                ...
                "TF_VAR_edge_junos_01_bootstrap_management_cidr": "192.168.4.40/24",
    ...
    def _verify_secondary_address(
        ...
        try:
            ...
            response = client.get(
                ...
                params={"address": cidr.split("/", 1)[0], "limit": 2},
            )
        except httpx.HTTPError:
            raise StagingError("NetBox staging address lookup failed") from None
        finally:
            if "client" in locals():
                client.close()
        if response.status_code != 200:
            raise StagingError("NetBox staging address lookup failed")
        try:
            payload = response.json()
        except ValueError:
            raise StagingError("NetBox staging address lookup failed") from None
        ...
        if (
            ...
            or results[0].get("address") != cidr
            or not isinstance(results[0].get("status"), dict)
            or results[0]["status"].get("value") != "active"
            or results[0].get("assigned_object_type") != "dcim.interface"
            or results[0].get("assigned_object_id") != interface_id
        ):
            raise StagingError("NetBox staging address authority mismatch")
    ...
    def create(self, evidence: StagingEvidence) -> None:
        ...
        if (
            ...
            or values.get("lab_id") == LEGACY_LAB
    ...
    def _render_edge(self) -> str:
        ...
        values = {
            ...
            "management_cidr": "192.168.4.40/24",
